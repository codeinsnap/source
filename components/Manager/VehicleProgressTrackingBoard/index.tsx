...
import { setLastVptbRefreshed } from "@/store/actions/commonAction";
...
export default function VPTBWrapper() {
  ...
  useEffect(()=>{
    if(lastVptbRefreshed !== lastRefreshedTime && intialApiCalled){
      const newData = JSON.parse(JSON.stringify(vehicleData?.vehilceTimeSheet) || '{}')

      setLastRefreshedTime(lastVptbRefreshed);
